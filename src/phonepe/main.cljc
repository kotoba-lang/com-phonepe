(ns phonepe.main
  "Kotodama WASM entrypoint for the Phonepe clean-room actor (L4) — Clojure port.
  ...
  facts (`phonepe.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "phonepe")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"phonepe.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "phonepe-compat" :tier tier :entities entities} 200])
...
(defn start! [] :phonepe-compat/ready)
